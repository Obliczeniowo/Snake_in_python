...
# Licence GPL-3.0 http://www.gnu.org/licenses/gpl-3.0.en.html
...
def main():
    ...
    class Snake: # snake class
        ...
        def draw(self):
            c_draw.delete("all")
            if self.col:
                c_draw.create_text([self.width / 2 * self.size, self.height / 2 * self.size], text = "Koniec gry.\nWęża o długości {w} otrzymałeś.".format(w = len(self.msnake)), font=("Arial", 20), justify = tk.CENTER)
